...
def main(argv_args):
    ...
    for fs in fileutils:      
        if not os.path.isfile(f"{train_folder}/{fs}"):
            print(os.path.isfile(f"{train_folder}/{fs}"))
    ...
    while feature_i < featureN:
        ...
        while eye_num < eye_max:
            eyeR_y_sum  = eyeR_y_sum + train_eye.iloc[0, eyeR_y_columns[eye_num]]
            eyeR_x_sum  = eyeR_x_sum + train_eye.iloc[0, eyeR_x_columns[eye_num]]
            eyeL_y_sum  = eyeL_y_sum + train_eye.iloc[0, eyeL_y_columns[eye_num]]
            eyeL_x_sum  = eyeL_x_sum + train_eye.iloc[0, eyeL_x_columns[eye_num]]
            eye_num    += 1
        ...
        eyeW_arys       = [train_eye.iloc[0, 3] - train_eye.iloc[0, 0], train_eye.iloc[0, 9] - train_eye.iloc[0, 6]] 
        eyeW            = np.amax(eyeW_arys) + (eyeW_alpha * 2)
        eyeH_arys       = [train_eye.iloc[0, 17] - train_eye.iloc[0, 14], train_eye.iloc[0, 22] - train_eye.iloc[0, 19]]
        eyeH            = np.amax(eyeH_arys) + (eyeH_alpha * 2)
        eye_size        = [eyeH, eyeW]
        eye_numel       = eyeH * eyeW
        ...
        while i < trainN:
            ...
            eye_num               = 0
            while eye_num < eye_max:
                eyeR_y_sum        = eyeR_y_sum + train_eye.iloc[eye_i, eyeR_y_columns[eye_num]]
                eyeR_x_sum        = eyeR_x_sum + train_eye.iloc[eye_i, eyeR_x_columns[eye_num]]
                eyeL_y_sum        = eyeL_y_sum + train_eye.iloc[eye_i, eyeL_y_columns[eye_num]]
                eyeL_x_sum        = eyeL_x_sum + train_eye.iloc[eye_i, eyeL_x_columns[eye_num]]
                eye_num += 1
        ...
        while i < testN:
            ...
            eye_num        = 0
            while eye_num < eye_max:
                eyeR_y_sum             = eyeR_y_sum + test_eye.iloc[eye_i, eyeR_y_columns[eye_num]]
                eyeR_x_sum             = eyeR_x_sum + test_eye.iloc[eye_i, eyeR_x_columns[eye_num]]
                eyeL_y_sum             = eyeL_y_sum + test_eye.iloc[eye_i, eyeL_y_columns[eye_num]]
                eyeL_x_sum             = eyeL_x_sum + test_eye.iloc[eye_i, eyeL_x_columns[eye_num]]
                eye_num               += 1
            ...
            trainY_std_mean            = np.add(trainY_std, trainY_mean)
            pre[i, :]                  = np.multiply(tmp, trainY_std_mean)
            
            # 四捨五入
            pre_col1                   = Decimal(pre[i, 1]) # pre行列の1列目
            pre_col2                   = Decimal(pre[i, 0]) # pre行列の2列目
        ...
        pr_args1               = np.nonzero(pre_round < 0)
        pre_round[pr_args1, :] = 0 # 教師用データの範囲外に出たものは丸める

        # pre_roundの値が"disp_size[0](1824)"より大きい場合
        pr_args2               = np.nonzero(pre_round[:, 0] > disp_size[0])
        pre_round[pr_args2, 0] = disp_size[0]

        # pre_roundの値が"disp_size[1](2736)"より大きい場合
        pr_args3               = np.nonzero(pre_round[:, 1] > disp_size[1])
        pre_round[pr_args3, 1] = disp_size[1]
    ...
    if pattern == 21:
        ...
        # 0の行を消す
        vip_mean_elem   = np.where(vip_mean[:, 1] == 0)
        vip_mean_arys   = np.array(vip_mean_elem[0])
        vip_mean_len    = len(vip_mean_arys)
        i               = 0
        del_num         = 0
        while i < vip_mean_len:
            vip_mean    = np.delete(vip_mean, vip_mean_arys[i], 0)
            del_num    += 1
            i += 1
